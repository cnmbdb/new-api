...
func UpsertPerfMetric(metric *PerfMetric) error {
	...
	return DB.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.Assignments(map[string]interface{}{
			"request_count":    gorm.Expr(`EXCLUDED."request_count" + ?`, metric.RequestCount),
			"success_count":    gorm.Expr(`EXCLUDED."success_count" + ?`, metric.SuccessCount),
			"total_latency_ms": gorm.Expr(`EXCLUDED."total_latency_ms" + ?`, metric.TotalLatencyMs),
			"ttft_sum_ms":      gorm.Expr(`EXCLUDED."ttft_sum_ms" + ?`, metric.TtftSumMs),
			"ttft_count":       gorm.Expr(`EXCLUDED."ttft_count" + ?`, metric.TtftCount),
			"output_tokens":    gorm.Expr(`EXCLUDED."output_tokens" + ?`, metric.OutputTokens),
			"generation_ms":    gorm.Expr(`EXCLUDED."generation_ms" + ?`, metric.GenerationMs),
		}),
	}).Create(metric).Error
}
