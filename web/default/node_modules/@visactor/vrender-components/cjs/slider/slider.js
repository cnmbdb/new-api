...
class Slider extends base_1.AbstractComponent {
    ...
    _renderHandlerText(value, position) {
        return vrender_core_1.graphicCreator.text(this._getHandlerTextAttributes(value, position));
    }
    _getHandlerPosition(isStart) {
        return this.attribute.range && isStart ? "start" : "end";
    }
    _getHandlerTextStyle(value, position) {
        const {align: align, handlerSize: handlerSize = 14, handlerText: handlerText = {}, railHeight: railHeight, railWidth: railWidth, slidable: slidable} = this.attribute;
        return (0, vutils_1.isFunction)(handlerText.style) ? handlerText.style(value, position, {
    ...
    _getHandlerTextAttributes(value, position) {
        var _a, _b;
        const {align: align, handlerSize: handlerSize = 14, handlerText: handlerText = {}, railHeight: railHeight, railWidth: railWidth, slidable: slidable} = this.attribute, isHorizontal = this._isHorizontal, pos = this.calculatePosByValue(value, position), textSpace = null !== (_a = handlerText.space) && void 0 !== _a ? _a : 4, handlerTextStyle = this._getHandlerTextStyle(value, position), textStyle = {
        ...
        return isHorizontal ? "top" === align ? (textStyle.textBaseline = "bottom", textStyle.textAlign = "center", 
        ...
        textStyle.x = (railWidth + handlerSize) / 2 + textSpace, textStyle.y = pos), Object.assign(Object.assign({}, textStyle), handlerTextStyle);
    }
    _renderTooltip() {
        var _a;
    ...
    _updateHandler(handler, position, value) {
        ...
        if (updateHandlerText) {
            const handlerPosition = this._getHandlerPosition(handler.name === constant_1.SLIDER_ELEMENT_NAME.startHandler);
    ...
    _updateHandlerText(handlerText, position, value) {
        const isHorizontal = this._isHorizontal, handlerPosition = this._getHandlerPosition(handlerText.name === constant_1.SLIDER_ELEMENT_NAME.startHandlerText);
