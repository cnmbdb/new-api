...
class FormatterPlugin extends base_plugin_1.BasePlugin {
    ...
    _formatSingleText(text, formatter) {
        ...
        return formatter.includes("%") && this._timeFormatter ? this._timeFormatter(formatter, text) : formatter.startsWith("calc(") ? this._calcFormatter(formatter, text) : text;
    }
    ...
    _calculateMathExpression(expression, text) {
        ...
        if (!this._isValidMathExpression(cleanExpression)) return text;
        try {
            const result = new Function("return (" + cleanExpression + ")")();
            if ("number" != typeof result || isNaN(result) || !isFinite(result)) throw new Error("Invalid calculation result");
            return result;
        } catch (error) {
            return text;
        }
