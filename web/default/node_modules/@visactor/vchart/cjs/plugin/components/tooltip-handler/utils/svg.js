...
function getSvgHtml(option, gradientId) {
    ...
    const styleString = 'style="display:inline-block;vertical-align:middle;"', {shapeType: shapeType, shapeStroke: shapeStroke, shapeHollow: shapeHollow = !1, shapeFill: shapeFill} = option, size = null !== (_a = option.shapeSize) && void 0 !== _a ? _a : 8, lineWidth = option.shapeLineWidth ? (0, 
    ...
    const isFillGradientStr = vrender_core_1.GradientParser.IsGradientStr(shapeFill);
    if (!shapeFill || (0, vutils_1.isString)(shapeFill) && !isFillGradientStr || shapeHollow) return fillString = shapeHollow ? "none" : shapeFill ? (0, 
    ...
    const shapeFillObject = isFillGradientStr ? vrender_core_1.GradientParser.Parse(shapeFill) : (0, 
    vutils_1.isObject)(shapeFill) ? shapeFill : null;
    if (shapeFillObject) {
        fillString = "gradientColor" + (null != gradientId ? gradientId : "");
        let gradient = "";
        const stops = (null !== (_b = shapeFillObject.stops) && void 0 !== _b ? _b : []).map((s => `<stop offset="${(0, 
