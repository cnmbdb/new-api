...
import { array, clamper, get, regressionLinear, regressionLogistic, regressionLowess, regressionPolynomial } from "@visactor/vchart";

import { SeriesTypeEnum } from "@visactor/vchart";

import { REGRESSION_LINE } from "../regression-line/regression-line";

const getRegressionByType = (type, data, x = (d => d.x), y = (d => d.y), degree) => {
    switch (type) {
      case "logisitc":
      ...
      case "polynomial":
        return regressionPolynomial(data, x, y, {
            degree: degree
        });
...
export function getScatterRegressionLineConfig(type, config) {
    ...
    return {
        ...
        style: {
            data: (datum, ctx) => {
                const series = ctx.vchart.getChart().getAllSeries().filter((s => s.type === SeriesTypeEnum.scatter)), regressionData = [];
                return series && series.length && series.forEach((s => {
                    var _a, _b;
                    const start = s.getRegion().getLayoutStartPoint(), rect = s.getRegion().getLayoutRect(), yClamper = clamper(start.y, start.y + rect.height), colorAttrOptions = s.getColorAttribute(), groups = s.getSeriesKeys(), data = s.getViewData().latestData, fieldX = null === (_a = s.fieldX) || void 0 === _a ? void 0 : _a[0], fieldY = null === (_b = s.fieldY) || void 0 === _b ? void 0 : _b[0];
...
export function appendScatterRegressionLineConfig(chartSpec, spec) {
    var _a, _b;
    spec || (spec = null !== (_a = get(chartSpec, REGRESSION_LINE)) && void 0 !== _a ? _a : get(null === (_b = chartSpec.series) || void 0 === _b ? void 0 : _b.find((s => s.type === SeriesTypeEnum.scatter)), REGRESSION_LINE));
