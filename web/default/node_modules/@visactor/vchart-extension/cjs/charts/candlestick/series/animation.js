...
const vchart_1 = require("@visactor/vchart"), vchart_2 = require("@visactor/vchart"), scaleIn = () => graphic => {
    ...
    return (0, vchart_1.isValidNumber)(open) && (0, vchart_1.isValidNumber)(close) && (open > close ? (animateAttributes.from.open = low, 
    animateAttributes.to.open = open, animateAttributes.from.close = low, animateAttributes.to.close = close, 
    (0, vchart_1.isValidNumber)(high) && (animateAttributes.from.high = low, animateAttributes.to.high = high)) : (animateAttributes.from.open = high, 
    animateAttributes.to.open = open, animateAttributes.from.close = high, animateAttributes.to.close = close, 
    (0, vchart_1.isValidNumber)(low) && (animateAttributes.from.low = high, animateAttributes.to.low = low))), 
    animateAttributes;
}, scaleOut = () => graphic => {
    const finalAttribute = graphic.getFinalAttribute(), {x: x, y: y, open: open, high: high, low: low, close: close} = finalAttribute, animateAttributes = {
        from: {
    ...
    return (0, vchart_1.isValidNumber)(open) && (0, vchart_1.isValidNumber)(close) && (open > close ? (animateAttributes.from.open = open, 
    animateAttributes.to.open = low, animateAttributes.from.close = close, animateAttributes.to.close = low, 
    (0, vchart_1.isValidNumber)(high) && (animateAttributes.from.high = high, animateAttributes.to.high = low)) : (animateAttributes.from.open = open, 
    animateAttributes.to.open = high, animateAttributes.from.close = close, animateAttributes.to.close = high, 
    (0, vchart_1.isValidNumber)(low) && (animateAttributes.from.low = low, animateAttributes.to.low = high))), 
    animateAttributes;
};

class CandlestickScaleIn extends vchart_2.ACustomAnimate {
...
class CandlestickScaleOut extends vchart_2.ACustomAnimate {
    constructor(from, to, duration, easing, params) {
        super(from, to, duration, easing, params);
    }
    onBind() {
...
const registerCandlestickScaleAnimation = () => {
    vchart_2.AnimateExecutor.registerBuiltInAnimate("candlestickScaleIn", CandlestickScaleIn), 
    vchart_2.AnimateExecutor.registerBuiltInAnimate("candlestickScaleOut", CandlestickScaleOut);
};
